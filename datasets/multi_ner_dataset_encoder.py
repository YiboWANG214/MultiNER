...
class MMNERDataset(Dataset):
    ...
    """
    def __init__(self, json_path_bert, json_path_characterbert, json_path, tokenizer: BertWordPieceTokenizer, max_length: int = 128, possible_only=False,
                 is_chinese=False, pad_to_maxlen=False):
        self.bert_embeddings = np.load(json_path_bert, allow_pickle=True)
        self.characterbert_embeddings = np.load(json_path_characterbert, allow_pickle=True)
    ...
    def __getitem__(self, item):
        ...
        Returns:
            ...
            label_idx: label id
        """
        bert_embedding = self.bert_embeddings[item]
        characterbert_embedding = self.characterbert_embeddings[item]
        ...
        # print("bert_embedding", type(bert_embedding))
        # print(bert_embedding.shape)
        return [
            bert_embedding,
            characterbert_embedding,
...
class MMNERDataset_fusion(Dataset):
    """
    MM NER Dataset
    Args:
        json_path: path to mrc-ner style json
        ...
        is_chinese: is chinese dataset
    """
    def __init__(self, json_path, tokenizer1: BertWordPieceTokenizer, tokenizer2: CharacterIndexer, max_length: int = 128, possible_only=False,
                 is_chinese=False, pad_to_maxlen=False):
        self.all_data = json.load(open(json_path, encoding="utf-8"))
    ...
    def __getitem__(self, item):
        ...
        type_ids = query_context_tokens.type_ids
        offsets = query_context_tokens.offsets
        text = query_context_tokens.tokens

        batch = [text]  # This is a batch with a single token sequence x
