...
from transformers import AdamW, get_linear_schedule_with_warmup, get_polynomial_decay_schedule_with_warmup, BertTokenizer
from torch.optim import SGD

# from datasets.mrc_ner_dataset import MMNERDataset
from datasets.multi_ner_dataset import MMNERDataset
from datasets.truncate_dataset import TruncateDataset
from datasets.collate_functions import collate_to_max_length, multi_collate_to_max_length
from metrics.query_span_f1 import QuerySpanF1, QueryExactF1
# from models.bert_query_ner import MMBertQueryNER
from models.multi_model2 import MMBertQueryNER
...
class BertLabeling(pl.LightningModule):
    def __init__(
        ...
        self.model = MMBertQueryNER.from_pretrained(args.bert_config_dir,
                                                  config=bert_config,
                                                #   from_tf=True,
                                                  )
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--classifier_intermediate_hidden_size", type=int, default=768)
    ...
    def compute_loss(self, start_logits, end_logits, span_logits,
        ...
        # print(start_labels.shape, match_labels.shape) # [batch_size, num_class, seq_len], [batch_size, num_class, seq_len, seq_len]
    ...
    def training_step(self, batch, batch_idx):
        ...
        # start_logits, end_logits, span_logits, a = self(tokens, attention_mask, token_type_ids)
        start_logits, end_logits, span_logits = self(tokens, attention_mask, token_type_ids)

        batch_size, num_class, seq_len = start_labels.size()
        # print(start_logits.shape, start_labels.shape, match_labels.shape, start_label_mask.shape)
        new_start_labels = []
        new_end_labels = []
        new_match_labels = []
        new_start_label_mask = []
        new_end_label_mask = []
        for i in range(batch_size):
            for j in range(num_class):
                new_start_labels.append(start_labels[i][j])
                new_end_labels.append(end_labels[i][j])
                new_match_labels.append(match_labels[i][j])
                new_start_label_mask.append(start_label_mask[i][j])
                new_end_label_mask.append(end_label_mask[i][j])
        start_labels = torch.stack(new_start_labels)
        end_labels = torch.stack(new_end_labels)
        match_labels = torch.stack(new_match_labels)
        start_label_mask = torch.stack(new_start_label_mask)
        end_label_mask = torch.stack(new_end_label_mask)
        # print(start_logits.shape, start_labels.shape, match_labels.shape, start_label_mask.shape)
    ...
    def test_step(self, batch, batch_idx):
        ...
        output["span_f1_stats"] = span_f1_stats
        # span_f1_stats = self.span_f1(start_preds=start_preds, end_preds=end_preds, match_logits=span_logits,
        #                              start_label_mask=start_labels, end_label_mask=end_labels,
        #                              match_labels=match_labels,
        #                              batch_idx=batch_idx
        #                              )
        # output["span_f1_stats"] = span_f1_stats
    ...
    def test_epoch_end(self, outputs) -> Dict[str, Dict[str, Tensor]]:
        ...
        all_counts = torch.stack([x[f'span_f1_stats'] for x in outputs]).view(-1, 3).sum(0)
        span_tp, span_fp, span_fn = all_counts
        print("span_tp, span_fp, span_fn", span_tp, span_fp, span_fn)
        span_recall = span_tp / (span_tp + span_fn + 1e-10)
        span_precision = span_tp / (span_tp + span_fp + 1e-10)
        span_f1 = span_precision * span_recall * 2 / (span_recall + span_precision + 1e-10)
        # label_count, pred_count, correct = all_counts
        # print("label_count, pred_count, correct", label_count, pred_count, correct)
        # span_recall = correct / (label_count + 1e-10)
        # span_precision = correct / (pred_count + 1e-10)
        # span_f1 = span_precision * span_recall * 2 / (span_recall + span_precision + 1e-10)
        ...
        # all_type_counts = torch.stack([x[f'span_f1_stats'][7:] for x in outputs]).view(-1, 3)
        # # print(all_type_counts.shape)
        # # print(all_type_counts)
        # type_counts = [0]*7
        # for i in range(0, len(all_type_counts)):
        #     type_counts[i%7] += all_type_counts[i][0]
        # print(type_counts)
        # for i in range(7):
        #     all_counts = torch.stack([x[f'span_f1_stats'][i] for x in outputs]).view(-1, 3).sum(0)
        #     span_tp, span_fp, span_fn = all_counts
        #     span_recall = span_tp / (span_tp + span_fn + 1e-10)
        #     span_precision = span_tp / (span_tp + span_fp + 1e-10)
        #     span_f1 = span_precision * span_recall * 2 / (span_recall + span_precision + 1e-10)
        #     # correct, pred_count, label_count = all_counts
        #     # print(correct, pred_count, label_count)
        #     # span_recall = correct / label_count
        #     # span_precision = correct / pred_count
        #     # span_f1 = span_precision * span_recall * 2 / (span_recall + span_precision + 1e-10)

        #     print(f"TEST INFO -> test_f1 for entity type {i} is: {span_f1} precision: {span_precision}, recall: {span_recall}")
    ...
    def get_dataloader(self, prefix="train", limit: int = None) -> DataLoader:
        ...
        # vocab_path = os.path.join("/mnt/WDRed4T/yibo/multi_task_NER/mrc-for-flat-nested-ner/BERT/biobert-base-v11", "vocab.txt")
        ...
        multi_dataset = []
        print("start")
        for i in range(3*7, 4*7, 7):
            multi_dataset.append([dataset[i+t] for t in range(7)])
        # multi_dataset = Dataset(multi_dataset)
        print("multi_dataset")
        ...
        dataloader = DataLoader(
            ...
            collate_fn=multi_collate_to_max_length
        )
